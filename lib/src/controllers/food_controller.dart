...
class FoodController extends ControllerMVC {
  ...
  bool isSameRestaurants(Food food) {
    if (carts.isNotEmpty) {
      return carts[0].clothes?.shop?.id == food.restaurant?.id;
    }
    return true;
  }
  ...
  Future<void> refreshFood() async {
    ...
    listenForFood(foodId: _id, message: S.of(context).clotheRefreshedSuccessfuly);
  }
