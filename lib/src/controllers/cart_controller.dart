...
class CartController extends ControllerMVC {
  ...
  void listenForCarts({String message}) async {
    ...
    stream.listen((Cart _cart) {
      if (!carts.contains(_cart)) {
        setState(() {
        //  coupon = _cart.shop.applyCoupon(coupon);
          carts.add(_cart);
        });
      }
    }, onError: (a) {
  ...
  void removeFromCart(Cart _cart) async {
    ...
    removeCart(_cart).then((value) {
      calculateSubtotal();
      scaffoldKey?.currentState?.showSnackBar(SnackBar(
        content: Text(S.of(context).the_clothe_was_removed_from_your_cart(_cart.clothes.name)),
      ));
    });
  }
  ...
  void calculateSubtotal() async {
    ...
    if (Helper.canDelivery(carts[0].clothes.shop, carts: carts)) {
      deliveryFee = carts[0].clothes.shop.delivery_fee;
    }
    taxAmount = (subTotal + deliveryFee) * carts[0].clothes.shop.default_tax / 100;
...
//          });
  ...
  void goCheckout(BuildContext context) {
    if (!currentUser.value.profileCompleted()) {
      ...
      if (/*carts[0].clothes.shop.closed*/true) {
        scaffoldKey?.currentState?.showSnackBar(SnackBar(
          content: Text(S.of(context).this_shop_is_closed_),
        ));
      } else {
