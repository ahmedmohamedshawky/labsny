...
class Order {
  String id;
  List<ClothesOrder> clothesOrders;
  ...
  Order.fromJSON(Map<String, dynamic> jsonMap) {
    try {
      ...
      clothesOrders = jsonMap['clothes_orders'] != null ? List.from(jsonMap['clothes_orders']).map((element) => ClothesOrder.fromJSON(element)).toList() : [];
    } catch (e) {
      id = '';
      tax = 0.0;
      deliveryFee = 0.0;
  ...
  Map toMap() {
    ...
    map["clothes"] = clothesOrders?.map((element) => element.toMap())?.toList();
