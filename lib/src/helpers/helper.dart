...
class Helper {
  ...
  static Future<Marker> getMarker(Map<String, dynamic> res, double totalDistance) async {
...
//        },
        ...
        infoWindow: InfoWindow(
            title: res['name'],
            // snippet: getDistance(totalDistance, setting.value.distanceUnit),
            snippet: getDistance(res['distance']!= null ?res['distance'].toDouble() :0.0, setting.value.distanceUnit),
            onTap: () {
  ...
  static double getTotalOrderPrice(ClothesOrder foodOrder) {
  ...
  static double getOrderPrice(ClothesOrder foodOrder) {
    double total = foodOrder.price;
    foodOrder.extras.forEach((extra) {
      total += extra.price != null ? extra.price : 0;
    });
  ...
  static double getTaxOrder(Order order) {
    double total = 0;
    order.clothesOrders.forEach((foodOrder) {
...
/**********************fgdffdfgdf*/
  static bool canDelivery(Shop _shop, {List<Cart> carts}) {
    bool _can = true;
    String _unit = setting.value.distanceUnit;
    double _deliveryRange = _shop.delivery_range;
    double _distance = _shop.distance;
    carts?.forEach((Cart _cart) {
      _can &= _cart.clothes.deliverable;
    });
    ...
    if (_distance == 0 && !deliveryAddress.value.isUnknown()) {
      _distance = sqrt(pow(69.1 * (double.parse(_shop.latitude) - deliveryAddress.value.latitude), 2) +
          pow(69.1 * (deliveryAddress.value.longitude - double.parse(_shop.longitude)) * cos(double.parse(_shop.latitude) / 57.3), 2));
    }
    _can &= _shop.available_for_delivery && (_distance < _deliveryRange) && !deliveryAddress.value.isUnknown();
    return _can;
  }


  static bool canDeliveryOffers(Offers _shop, {List<Cart> carts}) {
    bool _can = true;
    String _unit = setting.value.distanceUnit;
    double _deliveryRange = _shop.delivery_range;
    double _distance = _shop.distance;
